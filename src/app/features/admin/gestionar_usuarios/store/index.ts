...
const buildUsuariosQuery = (filters: UsuarioFilters = {}) => {
  ...
  if (
    filters.estadoConexion !== null &&
    filters.estadoConexion !== undefined
  ) {
  ...
  endpoints: (builder) => ({
    getUsuarios: builder.query<UsuarioPageResponse, UsuarioFilters | void>({
      ...
      providesTags: (result) => [
        ...
        ...(result?.content.map((item) => ({ type: "USUARIOS" as const, id: item.id })) ??
          []),
      ],
    }),
    getUsuarioById: builder.query<UsuarioListItem, string>({
